...
class Clubmeister(Base):
    __tablename__ = "clubmeister"
    __table_args__ = (UniqueConstraint("jahr", "rang", name="clubmeister_unique"),)
    ...
    werbungen: Mapped[int | None] = mapped_column(SmallInteger, nullable=True)
    mitglieddauer: Mapped[int | None] = mapped_column(SmallInteger, nullable=True)
    ...
    adressen: Mapped[Adressen] = relationship("Adressen", back_populates="clubmeister", lazy="selectin", join_depth=1)
